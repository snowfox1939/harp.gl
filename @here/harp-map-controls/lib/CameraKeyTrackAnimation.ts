...
export class CameraKeyTrackAnimation {
    ...
    private m_animateCb: () => void;
    private m_stopCb: () => void;
    ...
    constructor(private m_mapView: MapView, private m_options: CameraKeyTrackAnimationOptions) {
        ...
        const loop = this.m_options.loop !== undefined ? this.m_options.loop : THREE.LoopOnce;
        const repetitions =
            this.m_options.repetitions !== undefined ? this.m_options.repetitions : 1;

        this.m_options.rotateOnlyClockwise =
            this.m_options.rotateOnlyClockwise !== undefined
                ? this.m_options.rotateOnlyClockwise
                : true; //default to true

        this.m_name = this.m_options.name;
    ...
    start(time?: number, onFinished?: () => void): void {
        ...
        this.m_animationMixer.addEventListener("finished", this.m_stopCb);
    ...
    stop(): void {
        ...
        this.m_animationMixer.removeEventListener("finished", this.m_stopCb);
    ...
    private animate() {
        this.m_animationMixer.setTime((Date.now() - this.m_startTime) / 1000);
